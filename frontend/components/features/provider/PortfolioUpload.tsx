...
export function PortfolioUpload({ providerId, onUploadSuccess }: PortfolioUploadProps) {
	...
	const handleUpload = async () => {
		...
		try {
			await createPortfolioItem(providerId, { title, description: description || undefined, images: files });
	...
	return (
		<div className='bg-white rounded-lg shadow-md p-6 max-w-3xl mx-auto'>
			...
			{/* Image Upload Area */}
			<div className='mb-6'>
				<label className='block text-sm font-medium text-gray-700 mb-2'>Images * (1-10 images, max 5MB each)</label>
				...
				{files.length < 10 && (
					<div
						...
						:	<>
								...
								<p className='text-sm text-gray-500'>JPG, PNG, GIF, WEBP (Max 5MB per image, up to 10 images)</p>
							</>
						}
					</div>
				)}
				...
				{files.length > 0 && <p className='text-sm text-gray-600 mt-3'>{files.length} / 10 images selected</p>}
			</div>
