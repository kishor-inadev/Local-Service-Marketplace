...
describe("resolveId", () => {
  ...
  it("resolves display_id values to UUIDs", async () => {
    ...
    pool.query.mockResolvedValue({
      rows: [{ id: "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee" }],
    });
    ...
    expect(pool.query).toHaveBeenCalledWith(
      "SELECT id FROM jobs WHERE display_id = $1",
      ["JOBABC12345"],
    );
  });
  ...
  it("throws NotFoundException when display_id does not exist", async () => {
    ...
    await expect(resolveId(pool, "jobs", "JOBXXXXYYYY")).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it("throws for non-whitelisted table names", async () => {
    ...
    await expect(
      resolveId(pool, "unsafe_table", "JOBXXXXYYYY"),
    ).rejects.toThrow("resolveId: unknown table");
  });
});
