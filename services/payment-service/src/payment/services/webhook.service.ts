...
export class WebhookService {
	...
	async handleWebhook(
		...
		const isValid = this.paymentGateway.verifyWebhookSignature(gateway, rawBody, headers);
		if (!isValid) {
			this.logger.warn(`Webhook signature verification failed for gateway: ${gateway}`, "WebhookService");
	...
	private async processWebhook(webhook: PaymentWebhook, gateway: string): Promise<void> {
		try {
			this.logger.log(`Processing webhook ${webhook.id} from ${gateway}`, "WebhookService");
			...
			if (event.eventType === "unknown") {
				this.logger.log(`Unhandled webhook event type from ${gateway} — marking processed`, "WebhookService");
			...
			let payment =
				event.transactionId ? await this.paymentRepository.getPaymentByTransactionId(event.transactionId) : null;
			...
			if (event.eventType === "payment.succeeded") {
				await this.paymentRepository.updatePaymentStatus(payment.id, "completed", event.transactionId);
				this.logger.log(`Payment ${payment.id} marked completed via ${gateway} webhook`, "WebhookService");
				...
				if (userEmail) {
					this.notificationClient
						...
						.catch((err) => {
							this.logger.warn(`Failed to send payment confirmation email: ${err.message}`, "WebhookService");
						});
				}
			} else if (event.eventType === "payment.failed") {
				await this.paymentRepository.updatePaymentStatus(payment.id, "failed", event.transactionId);
				this.logger.log(`Payment ${payment.id} marked failed via ${gateway} webhook`, "WebhookService");
				...
				if (userEmail) {
					this.notificationClient
						...
						.catch((err) => {
							this.logger.warn(`Failed to send payment failure notification: ${err.message}`, "WebhookService");
						});
				}
			} else if (event.eventType === "refund.created") {
				await this.paymentRepository.updatePaymentStatus(payment.id, "refunded", event.transactionId);
				this.logger.log(`Payment ${payment.id} marked refunded via ${gateway} webhook`, "WebhookService");
			}
