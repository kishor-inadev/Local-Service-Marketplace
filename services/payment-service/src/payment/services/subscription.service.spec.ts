...
describe("SubscriptionService list validation", () => {
	const createService = () => {
		const subscriptionRepository = { findByProviderPaginated: jest.fn(), countByProvider: jest.fn() } as any;
	...
	it("rejects cursor with page for provider subscriptions", async () => {
		...
		await expect(
			service.getProviderSubscriptionsPaginated("provider-1", { cursor: "x", page: 2 } as any),
