...
export class SubscriptionRepository {
	...
	async findByProviderPaginated(providerId: string, queryDto: SubscriptionQueryDto): Promise<Subscription[]> {
		const {
			limit = 20,
			page,
			cursor,
	...
	async countByProvider(providerId: string, queryDto: SubscriptionQueryDto): Promise<number> {
	...
	async updateStatus(id: string, status: "active" | "cancelled" | "expired" | "pending"): Promise<Subscription> {
		const query = `
	...
	async getExpiringSubscriptionsPaginated(days: number, queryDto: SubscriptionQueryDto): Promise<Subscription[]> {
		const { limit = 20, page, status, sortBy = SubscriptionSortBy.EXPIRES_AT, sortOrder = SortOrder.ASC } = queryDto;
		let query = `
      SELECT s.*, p.business_name, p.user_id, pl.name as plan_name
      FROM subscriptions s
      JOIN providers p ON s.provider_id = p.id
	...
	async countExpiringSubscriptions(days: number, queryDto: SubscriptionQueryDto): Promise<number> {
		const { status } = queryDto;
		let query = `
