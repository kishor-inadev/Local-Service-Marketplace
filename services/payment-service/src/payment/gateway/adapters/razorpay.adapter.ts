...
export class RazorpayAdapter implements IGatewayAdapter {
	...
	async charge(params: ChargeParams): Promise<ChargeResult> {
		...
		this.logger.log(`Razorpay Order created: ${order.id}, status: ${order.status}`);
	...
	async refund(params: RefundParams): Promise<RefundResult> {
		...
		const refund = await this.razorpay.payments.refund(params.transactionId, refundBody);
	...
	async getPaymentStatus(transactionId: string): Promise<ChargeResult> {
		...
		try {
			...
			const orderPayments = await this.razorpay.orders.fetchPayments(transactionId);
			payment = orderPayments?.items?.[0] ?? {};
		}

		const razorpayStatus = payment.status as string;
		const normalized =
			razorpayStatus === "captured" ? "succeeded"
			: razorpayStatus === "failed" ? "failed"
			: "pending";
	...
	parseWebhookEvent(payload: Record<string, any>): ParsedWebhookEvent {
		...
		const paymentEntity: Record<string, any> = payload.payload?.payment?.entity ?? {};
		const refundEntity: Record<string, any> = payload.payload?.refund?.entity ?? {};
		...
		switch (event) {
			...
			case "refund.processed":
				return { eventType: "refund.created", transactionId: refundEntity.payment_id };
