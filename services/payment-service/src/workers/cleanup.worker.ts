...
export class PaymentCleanupWorker extends WorkerHost implements OnModuleInit {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    try {
      ...
      this.logger.error(`Job "${job.name}/${job.id}" threw: ${err.message}`, err.stack, 'PaymentCleanupWorker');
      throw error;
    }
  }
  ...
  onActive(job: Job): void {
    this.logger.log(`Job "${job.name}/${job.id}" started (attempt ${job.attemptsMade + 1})`, 'PaymentCleanupWorker');
  }

  @OnWorkerEvent('completed')
  onCompleted(job: Job): void {
    this.logger.log(`Job "${job.name}/${job.id}" completed`, 'PaymentCleanupWorker');
  }
  ...
  onError(error: Error): void {
    this.logger.error(`Worker error: ${error.message}`, error.stack, 'PaymentCleanupWorker');
  }

  @OnWorkerEvent('stalled')
  onStalled(jobId: string): void {
    this.logger.warn(`Job ${jobId} stalled and will be requeued`, 'PaymentCleanupWorker');
  }
}
