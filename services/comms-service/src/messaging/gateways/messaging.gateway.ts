...
export class MessagingGateway implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit {
	...
	async handleSendMessage(@ConnectedSocket() client: AuthenticatedSocket, @MessageBody() data: CreateMessageDto) {
		try {
	...
	handleTyping(
		...
		this.server.to(`user:${data.receiverId}`).emit("message:typing", { senderId, isTyping: data.isTyping });
	...
	async handleMarkAsRead(@ConnectedSocket() client: AuthenticatedSocket, @MessageBody() data: { messageId: string }) {
		try {
			const userId = client.userId;
	...
	handleGetOnlineStatus(@ConnectedSocket() client: AuthenticatedSocket, @MessageBody() data: { userIds: string[] }) {
		const userId = client.userId;
	...
	private verifyJwt(token: string): any {
		try {
			...
			const expectedSig = crypto.createHmac("sha256", this.jwtSecret).update(signatureInput).digest("base64url");
