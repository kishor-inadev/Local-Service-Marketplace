...
export class MetricsInterceptor implements NestInterceptor {
  ...
  static getMetrics() {
    const routes: Record<
      string,
      { count: number; errors: number; avgMs: number }
    > = {};
    ...
    return {
      ...
      uptimeSeconds: Math.round(
        (Date.now() - MetricsInterceptor.startTime) / 1000,
      ),
  ...
  static toPrometheus(): string {
    ...
    lines.push(
      `process_uptime_seconds ${Math.round((Date.now() - m.startTime) / 1000)}`,
    );
    ...
    lines.push(
      "# HELP http_request_duration_ms_avg Average request duration per route",
    );
    lines.push("# TYPE http_request_duration_ms_avg gauge");
    for (const [key, metric] of m.metrics.entries()) {
      const label = key.replace(/"/g, '\\"');
      const avg =
        metric.count > 0 ? Math.round(metric.totalMs / metric.count) : 0;
